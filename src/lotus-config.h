...
inline LotusMode modeStringToEnum(const std::string &mode) {
    static const std::unordered_map<std::string_view, LotusMode> modeMap = {
        {"uinput", LotusMode::Uinput},
        {"surrounding", LotusMode::SurroundingText},
        {"preedit", LotusMode::Preedit},
        {"uinputhc", LotusMode::UinputHC},
        {"Off", LotusMode::Off},
        {"emoji", LotusMode::Emoji},
    ...
    void dumpDescription(RawConfig &config) const {
        ...
        for (size_t i = 0; i < list_.size(); ++i) {
            config.setValueByPath(
                ...
                stringutils::concat("fcitx://config/addon/lotus/macro/",
                                    list_[i]));
        }
    }
};
...
FCITX_CONFIGURATION(lotusKeymap,
                    Option<std::string> key{this, "Key", _("Key"), ""};
                    Option<std::string> value{this, "Value", _("Value"), ""};);

FCITX_CONFIGURATION(
    lotusMacroTable,
    OptionWithAnnotation<std::vector<lotusKeymap>, ListDisplayOptionAnnotation>
        macros{this,
               "Macro",
               _("Macro"),
               {},
...
FCITX_CONFIGURATION(
    ...
    OptionWithAnnotation<std::string, ModeListAnnotation> mode{
        this, "Mode", _("Mode"), "smooth", {}, {}, ModeListAnnotation()};
    Option<std::string, InputMethodConstrain, DefaultMarshaller<std::string>,
           InputMethodAnnotation>
        inputMethod{this,
                    "InputMethod",
    ...
    Option<bool> modernStyle{this, "ModernStyle",
                             _("Use oà, _uý (instead of òa, úy)"), true};
    Option<bool> freeMarking{this, "FreeMarking",
                             _("Allow type with more freedom"), true};
    Option<bool> fixUinputWithAck{this, "FixUinputWithAck",
                                  _("Fix uinput mode with ack"), false};
    SubConfigOption customKeymap{this, "CustomKeymap", _("Custom Keymap"),
                                 "fcitx://config/addon/lotus/custom_keymap"};);
